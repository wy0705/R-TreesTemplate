...
// Fwd decl
class RTFileStream;  // File I/O helper class, look below for implementation and notes.文件I/O助手类，请查看下面的实现和注释。
...
///用法示例：对于三维树，使用RTree<Object*，float，3>myTree；

///

///此修改后的模板化C++版本由Greg Douglas在Auran（http://www.auran.com)

///

///数据类型引用数据，应为int、void*、obj*等。不大于sizeof<void*>和simple type

///ELEMTYPE元素的类型，如int或float

///NUMDIMS尺寸数，如2或3

///ELEMTYPEREAL一种允许在体积计算中使用分数和大值（如浮点或双精度）的元素

///

///注意：插入和删除数据需要了解其恒定最小边界矩形。

///这个版本对节点使用new/delete，为了提高效率，我建议使用固定大小的分配器。

///我建议不要对返回的结果使用回调函数，而是高效地预先调整大小，只增加内存

///类似于MFC CArray或STL Vector的数组，用于返回搜索查询结果。*/
/// \tparam DATATYPE
/// \tparam ELEMTYPE
/// \tparam NUMDIMS
/// \tparam ELEMTYPEREAL
...
public:
  ...
  private:
    ...
    enum { MAX_STACK = 32 }; //  Max stack size. Allows almost n^32 where n is number of branches in node最大堆栈大小。几乎允许n^32，其中n是节点中的分支数
